...
export const listJobsSchema = z.object({ q: text(120).optional(), category: z.enum(categories).optional(), budgetType: z.enum(['fixed', 'hourly']).optional(), experienceLevel: z.enum(['entry', 'intermediate', 'expert']).optional(), duration: z.enum(['short', 'medium', 'long']).optional(), activity: z.enum(['applied', 'followed']).optional(), sort: z.enum(['recent', 'budget_asc', 'budget_desc']).optional(), page: z.coerce.number().int().min(1).optional(), limit: z.coerce.number().int().min(1).max(50).optional() }).strict();
